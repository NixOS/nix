...
> PR_COMMENT There should be an entirely new page that would be linked here about how to determine the type of install one has (e.g., if Nix was installed via `snap` or `apt` on ubuntu, is it multi or single?). There should also be a warning section here that if someone built Nix from source and set up supporting infrastructure then they should enumerate the steps and do their inverse (if exists), but we can't provide more information.
