...
let
  ...
  externalSanitizerLibcxx = mkFixture {
    name = "fuzzing-engine-external-sanitizer-libcxx";
    stdenv = pkgsLLVM.stdenv;
    src = "${fixture}/subprojects/util";
    executables = [ "util" ];
    fuzzingEngine = "-fsanitize=fuzzer";
    runArgs = "-runs=1";
...
linkFarm "fuzzing-engine" [
  ...
  {
    name = "external-sanitize-fuzzer-libcxx";
    path = externalSanitizerLibcxx;
  }
  {
