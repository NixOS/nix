...
# the correct one
CORRECT_ADDRESS=$( \
    nix eval --raw --impure --expr "builtins.fetchurl \"file://$PWD/$TEST_FILE\"" --store ipfs://$EMPTY_HASH?allow-modify=true \
...
RESULT=$(nix eval --json --impure --expr '(builtins.fetchurl 'file://$PWD/$TEST_FILE')' --store "$CORRECT_ADDRESS" \
    | jq -r | awk -F/ '{print $NF}')
