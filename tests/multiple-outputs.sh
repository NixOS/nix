...
# ‘drvPath’ attribute.
echo "evaluating c..."
#drvPath=$(nix-instantiate $NIX_TEST_ROOT/multiple-outputs.nix -A c --readonly-mode)

# And check whether the resulting derivation explicitly depends on all
# outputs.
drvPath=$(nix-instantiate $NIX_TEST_ROOT/multiple-outputs.nix -A c)
#[ "$drvPath" = "$drvPath2" ]
grep -q 'multiple-outputs-a.drv",\["first","second"\]' $drvPath
grep -q 'multiple-outputs-b.drv",\["out"\]' $drvPath

# While we're at it, test the ‘unsafeDiscardOutputDependency’ primop.
outPath=$(nix-build $NIX_TEST_ROOT/multiple-outputs.nix -A d --no-out-link)
...
# outputs.
echo "building b..."
outPath=$(nix-build $NIX_TEST_ROOT/multiple-outputs.nix -A b --no-out-link)
echo "output path is $outPath"
[ "$(cat "$outPath"/file)" = "success" ]

# Test nix-build on a derivation with multiple outputs.
nix-build $NIX_TEST_ROOT/multiple-outputs.nix -A a -o $TEST_ROOT/result
[ -e $TEST_ROOT/result-first ]
(! [ -e $TEST_ROOT/result-second ])
nix-build $NIX_TEST_ROOT/multiple-outputs.nix -A a.all -o $TEST_ROOT/result
...
nix-build $NIX_TEST_ROOT/multiple-outputs.nix -A a.first --no-out-link

# Cyclic outputs should be rejected.
echo "building cyclic..."
if nix-build $NIX_TEST_ROOT/multiple-outputs.nix -A cyclic --no-out-link; then
