...
nix_gc_closure() {
    ...
    if isDaemonNewer "2.35pre"; then
        # Check that nix store delete --recursive --ignore-alive is best-effort (doesn't fail when some paths in the closure are alive)
        nix store delete --recursive --ignore-alive "$top"
        ...
        expectStderr 1 nix store delete --recursive --ignore-alive "$top" | grepQuiet "Your daemon version is too old to support garbage collecting a closure"
    fi
}
