...
# Test a repository with a submodule located outside of the parent repository
parentrepo=$TEST_ROOT/git
subrepo=$TEST_ROOT/subgit

rm -rf "$parentrepo" "${repo}-tmp" "$TEST_HOME/.cache/nix"

git init --initial-branch="master" "$parentrepo"
git -C "$parentrepo" config user.email "foobar@example.com"
git -C "$parentrepo" config user.name "Foobar"

echo "hello world" > "$parentrepo/hello_world"
git -C "$parentrepo" add hello_world
git -C "$parentrepo" commit -m 'My first commit.'

git init --initial-branch="master" "$subrepo"
git -C "$subrepo" config user.email "foobar@example.com"
git -C "$subrepo" config user.name "Foobar"

echo "hello world" > "$subrepo/hello_world"
git -C "$subrepo" add hello_world
git -C "$subrepo" commit -m 'My first commit.'

git -c protocol.file.allow=always -C "$parentrepo" submodule add "$subrepo" sub
git -C "$parentrepo" commit -m 'add submodule'

path13=$(nix eval --impure --raw --expr "(builtins.fetchGit $parentrepo).outPath" --tarball-ttl 0)

export _NIX_FORCE_HTTP=1
path14=$(nix eval --impure --raw --expr "(builtins.fetchGit $parentrepo).outPath" --tarball-ttl 0)
unset _NIX_FORCE_HTTP

# store paths of git repository should include the empty submodule directories generated by git
# see https://github.com/NixOS/nix/issues/13698
[[ -d "$path13"/sub ]]
[[ -d "$path14"/sub ]]

shopt -s nullglob dotglob
subrepofiles13=("$path13"/sub/*)
subrepofiles14=("$path14"/sub/*)
shopt -u nullglob dotglob

# check that the submodule directory is empty since `submodules=0` is implicitly specified
[[ ${#subrepofiles13[@]} -eq 0 ]]
[[ ${#subrepofiles14[@]} -eq 0 ]]
