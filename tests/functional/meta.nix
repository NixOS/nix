...
rec {
  ...
  # Derivation where derivation-meta is in requiredSystemFeatures along with
  # other features. Verify that only derivation-meta is filtered from the output
  # hash, not the others: its output path must equal `otherFeaturesNoMeta`, which
  # keeps the same features minus derivation-meta and __meta.
  # `kvm` sorts after `derivation-meta`, so this also exercises re-splicing
  # derivation-meta back into the *middle* of a sorted requiredSystemFeatures
  # list on serialisation.
  ...
  # Same as `metaWithOtherFeatures` but without __meta and the derivation-meta
  # feature. Since derivation-meta and __meta are filtered from the output hash,
  # this must produce the same output path as `metaWithOtherFeatures`.
  otherFeaturesNoMeta = mkDerivation {
    name = "meta-test";
    __structuredAttrs = true;
    buildCommand = ''
      echo "Hello from meta test" > "''${outputs[out]}"
  ...
  # requiredSystemFeatures must be sorted when using derivation-meta, so this
  # must be rejected at instantiation time.
  metaWithUnsortedFeatures = mkDerivation {
    name = "meta-test";
    __structuredAttrs = true;
    buildCommand = ''
      echo "Hello from meta test" > "''${outputs[out]}"
    '';
    __meta = {
      description = "With unsorted features";
    };
    requiredSystemFeatures = [
      "big-parallel"
      "derivation-meta"
  ...
  # A duplicate entry (in an otherwise sorted list) must be rejected, and
  # reported as a duplicate rather than as "unsorted".
  metaWithDuplicateFeatures = mkDerivation {
    name = "meta-test";
    __structuredAttrs = true;
    buildCommand = ''
      echo "Hello from meta test" > "''${outputs[out]}"
    '';
    __meta = {
      description = "With duplicate features";
    };
    requiredSystemFeatures = [
      "big-parallel"
      "big-parallel"
