...
# Regression test: a locked flake input must re-evaluate without a "mismatch in
# field 'revCount'" error. The lock file stores revCount as a plain integer,
# while a fresh fetch computes it lazily; re-checking the lock must compare the
# resolved values, not clobber the locked integer with the lazy attribute.
dep=$TEST_ROOT/jj-dep
jj git init "$dep" >/dev/null
echo '{ outputs = _: { v = 42; }; }' > "$dep"/flake.nix
jj --repository "$dep" describe -m c1 >/dev/null
jj --repository "$dep" new -m c2 >/dev/null   # a second commit so revCount > 1
depRev=$(jj --repository "$dep" --ignore-working-copy log -r @- --no-graph -T commit_id)

flakeDir=$TEST_ROOT/jj-flake
mkdir -p "$flakeDir"
cat > "$flakeDir"/flake.nix <<EOF
{
  inputs.dep.url = "jj+file://$dep?rev=$depRev";
  outputs = { self, dep }: { inherit (dep) v revCount; };
}
EOF
nix flake lock "$flakeDir"
# The lock records revCount as a plain integer.
grepQuiet '"revCount":' "$flakeDir"/flake.lock
# Clearing the store forces the locked (final) input to be re-fetched rather than
# reused from the store or a substituter, which is what exercises the lock check
# against the fetcher's freshly computed (lazy) revCount.
clearStore
[[ $(nix eval "$flakeDir"#v) = 42 ]]
[[ $(nix eval "$flakeDir"#revCount) -ge 1 ]]
