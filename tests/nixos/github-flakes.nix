...

{
  ...
  testScript = { nodes }: ''
    ...
    hash = client.succeed(f"nix eval --no-trust-github --raw --expr '(fetchTree {info['url']}).narHash'")
    ...
    out = client.fail(f"nix eval --no-trust-github --raw --expr 'builtins.fetchTree github:github:fancy-enterprise/private-flake/{info['revision']}' 2>&1")
    assert "will not fetch unlocked input" in out, "--no-trust-github did not fail with the expected error"
