  ...
  testScript =
    ...
    ''
      ...
      (status, output) = machine.execute(
          ...
          "  ${pkgs.strace}/bin/strace -f -e trace=mount -o /strace.log "
          "    ${nix}/bin/nix store info --store local "
          "      --extra-experimental-features nix-command"
          "'"
      )

      print(machine.succeed("cat /tmp/container/strace.log"))

      assert status == 0, f"nspawn failed (status {status}): {output}"
      machine.succeed("${pkgs.gnugrep}/bin/grep -q MS_NODEV /tmp/container/strace.log")
    '';
}
