...
#include "unpack-channel.nix.gen.hh"
    ...
    for (const auto & channel : channels) {
        ...
        if (!(channelNames.empty() || channelNames.count(name))) {
            ...
            if (std::regex_search(filename, std::regex("\\.tar\\.(gz|bz2|xz)$"))) {
                runProgram(
                    settings.nixBinDir + "/nix-build", false,
                    {"--no-out-link", "--expr",
            ...
            if (!unpacked) {
                ...
                try {
                    filename = store->toRealPath(
                        fetchers::downloadFile(store, url + "/nixexprs.tar.xz", "nixexprs.tar.xz", false).storePath);
                } catch (FileTransferError & e) {
                    filename = store->toRealPath(
                        fetchers::downloadFile(store, url + "/nixexprs.tar.bz2", "nixexprs.tar.bz2", false).storePath);
                }
            }
    ...
    Strings envArgs{"--profile", profile,        "--file",           unpackChannelPath,
                    "--install", "--remove-all", "--from-expression"};
    for (auto & expr : exprs)
