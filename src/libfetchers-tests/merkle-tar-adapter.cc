...
TEST_F(MerkleTarAdapterTest, hardlink_target_not_found)
{
    ...
    EXPECT_THAT(
        ...
        ::testing::ThrowsMessage<Error>(::testing::AllOf(
            ...
            ::testing::HasSubstr("nonexistent"),
            ::testing::HasSubstr("link"))));
}
...
TEST_F(MerkleTarAdapterTest, hardlink_to_self_fails)
{
    auto repo = openRepo();
    auto pool = openWriterPool();
    auto tarSink = merkle::makeTarSink(*pool);

    /* Here we are more strict than libarchive, which only warns on cyclic hard links. */
    tarSink->createHardlink(CanonPath("foo"), CanonPath("foo"));

    EXPECT_THAT([&]() { tarSink->flush(); }, ::testing::ThrowsMessage<Error>(::testing::HasSubstr("cycle")));
}

TEST_F(MerkleTarAdapterTest, hardlink_cycle_fails)
{
    auto repo = openRepo();
    auto pool = openWriterPool();
    auto tarSink = merkle::makeTarSink(*pool);

    tarSink->createHardlink(CanonPath("foo"), CanonPath("bar"));
...
TEST_F(MerkleTarAdapterTest, replacing_empty_directory)
{
    ...
    auto accessor = repo->getAccessor(result.hash, {}, getRepoName());
    ASSERT_EQ(accessor->readDirectory(CanonPath("foo")).size(), 1u);
    ASSERT_EQ(accessor->readFile(CanonPath("foo/bar")), "test");
}
...
TEST_F(MerkleTarAdapterTest, replacing_non_empty_directory_fails)
{
    ...
    EXPECT_THAT(
        ...
        ::testing::ThrowsMessage<Error>(::testing::HasSubstr("conflicting non-empty directory")));
}
