...
namespace nix {
    ...
    TEST_F(PrimOpTest, attrValues) {
        ...
        ASSERT_THAT(*v.list()[0], IsIntEq(1));
        ASSERT_THAT(*v.list()[1], IsStringEq("foo"));
    }
    ...
    TEST_F(PrimOpTest, catAttrs) {
        ...
        ASSERT_THAT(v, IsListOfSize(2));
        ASSERT_THAT(*v.list()[0], IsIntEq(1));
        ASSERT_THAT(*v.list()[1], IsIntEq(2));
    }
    ...
    TEST_F(PrimOpTest, tail) {
        ...
        for (const auto [n, elem] : enumerate(v.list()))
    ...
    TEST_F(PrimOpTest, concatLists) {
        ...
        for (const auto [i, elem] : enumerate(v.list()))
    ...
    TEST_F(PrimOpTest, partition) {
        ...
        ASSERT_THAT(*right->value->list()[0], IsIntEq(23));
        ASSERT_THAT(*right->value->list()[1], IsIntEq(42));
        ...
        ASSERT_THAT(*wrong->value->list()[0], IsIntEq(1));
        ASSERT_THAT(*wrong->value->list()[1], IsIntEq(9));
        ASSERT_THAT(*wrong->value->list()[2], IsIntEq(3));
    }
...
#undef CASE
    ...
    TEST_F(PrimOpTest, splitVersion) {
        ...
        for (const auto [n, p] : enumerate(v.list()))
...
#undef CASE
  ...
  class ParseDrvNamePrimOpTest :
    ...
    TEST_F(PrimOpTest, split1) {
        ...
        ASSERT_THAT(*v.list()[0], IsStringEq(""));

        ASSERT_THAT(*v.list()[1], IsListOfSize(1));
        ASSERT_THAT(*v.list()[1]->list()[0], IsStringEq("a"));

        ASSERT_THAT(*v.list()[2], IsStringEq("c"));
    }
    ...
    TEST_F(PrimOpTest, split2) {
        ...
        ASSERT_THAT(*v.list()[2], IsStringEq("b"));

        ASSERT_THAT(*v.list()[3], IsListOfSize(1));
        ASSERT_THAT(*v.list()[3]->list()[0], IsStringEq("c"));
    ...
    TEST_F(PrimOpTest, split3) {
        ...
        ASSERT_THAT(*v.list()[1], IsListOfSize(2));
        ASSERT_THAT(*v.list()[1]->list()[0], IsStringEq("a"));
        ASSERT_THAT(*v.list()[1]->list()[1], IsNull());

        // 3rd element
        ASSERT_THAT(*v.list()[2], IsStringEq("b"));

        // 4th element is a list: [ null "c" ]
        ASSERT_THAT(*v.list()[3], IsListOfSize(2));
        ASSERT_THAT(*v.list()[3]->list()[0], IsNull());
        ASSERT_THAT(*v.list()[3]->list()[1], IsStringEq("c"));
    ...
    TEST_F(PrimOpTest, match3) {
        ...
        ASSERT_THAT(v, IsListOfSize(2));
        ASSERT_THAT(*v.list()[0], IsStringEq("b"));
        ASSERT_THAT(*v.list()[1], IsStringEq("c"));
    }

    TEST_F(PrimOpTest, match4) {
        auto v = eval("builtins.match \"[[:space:]]+([[:upper:]]+)[[:space:]]+\" \"  FOO   \"");
        ASSERT_THAT(v, IsListOfSize(1));
        ASSERT_THAT(*v.list()[0], IsStringEq("FOO"));
    }
