    ...
    Store & store, const SingleDerivedPath & path, Store * evalStore_, QueryRealisationFun & queryRealisation)
{
    ...
    auto & built = std::get<SingleDerivedPath::Built>(path.raw());
    auto innerPath = resolveSingleDerivedPath(store, *built.drvPath, evalStore_, queryRealisation);
    if (!innerPath)
        return std::nullopt;
    return deepQueryPartialDerivationOutput(store, *innerPath, built.output, evalStore_, queryRealisation).outPath;
}
