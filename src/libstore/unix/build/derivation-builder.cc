...
std::variant<std::pair<BuildResult::Status, Error>, SingleDrvOutputs> DerivationBuilderImpl::unprepareBuild()
{
    ...
    try {
        ...
        if (!statusOk(status)) {
            ...
            auto msg = fmt("builder for '%s' %s", Magenta(store.printStorePath(drvPath)), statusToString(status));
...
void DerivationBuilderImpl::startDaemon()
{
    ...
    auto store = makeRestrictedStore(
        params, ref<LocalStore>(std::dynamic_pointer_cast<LocalStore>(this->store.shared_from_this())), *this);
        ...
        if (drv.isBuiltin()) {
            try {
                ...
                for (auto & e : drv.outputs)
                    outputs.insert_or_assign(e.first, store.printStorePath(scratchOutputs.at(e.first)));
