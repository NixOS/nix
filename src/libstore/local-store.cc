...
void LocalStore::readSubstitutableFile(const Path & path, FdPair & fds)
{
    bool foundSub;
    foreach (Paths::iterator, i, settings.substituters) {
        debug(format("starting substituter program `%1%'") % *i);
        ...
        case 0: /* child */
            try {
                ...
                if (dup2(errorPipe.writeSide, STDERR_FILENO) == -1)
                    throw SysError("dupping stderr");
                closeMostFDs(set<int>());
                execl(i->c_str(), i->c_str(), "--read", path.c_str(), NULL);
                throw SysError(format("executing `%1%'") % *i);
            } catch (std::exception & e) {
                std::cerr << "error: " << e.what() << std::endl;
            }
            quickExit(1);
        ...
        char buf;
        if (!read(fromPipe.readSide,&buf,sizeof buf))
            continue;
    ...
    if (!foundSub)
        throw Error(format("Could not read `%1%' from any substituter") % path);
}
