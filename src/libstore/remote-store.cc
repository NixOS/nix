...
StorePathSet RemoteStore::queryValidPaths(const StorePathSet & paths, SubstituteFlag maybeSubstitute)
{
    ...
    if (GET_PROTOCOL_MINOR(conn->daemonVersion) < 12) {
        ...
        worker_proto::write(*this, *conn, paths);
        if (GET_PROTOCOL_MINOR(conn->daemonVersion) >= 27) {
            conn->to << (settings.buildersUseSubstitutes ? 1 : 0);
        }
        conn.processStderr();
        return worker_proto::read(*this, *conn, Phantom<StorePathSet> {});
    }
}
...
void RemoteStore::querySubstitutablePathInfos(const StorePathCAMap & pathsMap, SubstitutablePathInfos & infos)
{
    ...
    if (GET_PROTOCOL_MINOR(conn->daemonVersion) < 12) {
        ...
        for (auto & i : pathsMap) {
            ...
            info.references = worker_proto::read(*this, *conn, Phantom<StorePathSet> {});
        ...
        if (GET_PROTOCOL_MINOR(conn->daemonVersion) < 22) {
            ...
            worker_proto::write(*this, *conn, pathsMap);
        conn.processStderr();
    ...
    StorePathSet & referrers)
{
    ...
    conn.processStderr();
    for (auto & i : worker_proto::read(*this, *conn, Phantom<StorePathSet> {}))
        referrers.insert(i);
}
...
std::map<std::string, std::optional<StorePath>> RemoteStore::queryPartialDerivationOutputMap(const StorePath & path)
{
    if (GET_PROTOCOL_MINOR(getProtocol()) >= 0x16) {
        ...
        conn.processStderr();
        return worker_proto::read(*this, *conn, Phantom<std::map<std::string, std::optional<StorePath>>> {});
    } else {
    ...
    RepairFlag repair)
{
    ...
    if (GET_PROTOCOL_MINOR(conn->daemonVersion) >= 25) {
        ...
        worker_proto::write(*this, *conn, references);
        conn->to << repair;
    ...
    RepairFlag repair, CheckSigsFlag checkSigs)
{
    ...
    if (GET_PROTOCOL_MINOR(conn->daemonVersion) < 18) {
        ...
        auto importedPaths = worker_proto::read(*this, *conn, Phantom<StorePathSet> {});
...
void RemoteStore::registerDrvOutput(const Realisation & info)
{
    ...
    if (GET_PROTOCOL_MINOR(conn->daemonVersion) < 31) {
        ...
        worker_proto::write(*this, *conn, info);
    }
    conn.processStderr();
}
    ...
    auto real = [&]() -> std::shared_ptr<const Realisation> {
        if (GET_PROTOCOL_MINOR(conn->daemonVersion) < 31) {
            auto outPaths = worker_proto::read(
                *this, *conn, Phantom<std::set<StorePath>> {});
            if (outPaths.empty())
                return nullptr;
            return std::make_shared<const Realisation>(Realisation { .id = id, .outPath = *outPaths.begin() });
        } else {
            auto realisations = worker_proto::read(
                *this, *conn, Phantom<std::set<Realisation>> {});
...
static void writeDerivedPaths(RemoteStore & store, RemoteStore::Connection & conn, const std::vector<DerivedPath> & reqs)
{
    if (GET_PROTOCOL_MINOR(conn.daemonVersion) >= 30) {
        worker_proto::write(store, conn, reqs);
    } else {
        Strings ss;
        for (auto & p : reqs) {
    ...
    std::shared_ptr<Store> evalStore)
{
    ...
    if (GET_PROTOCOL_MINOR(conn->daemonVersion) >= 34) {
        ...
        conn->to << buildMode;
        conn.processStderr();
        return worker_proto::read(*this, *conn, Phantom<std::vector<KeyedBuildResult>> {});
    } else {
        // Avoid deadlock.
        conn_.reset();
    ...
    BuildMode buildMode)
{
    ...
    if (GET_PROTOCOL_MINOR(conn->daemonVersion) >= 28) {
        auto builtOutputs = worker_proto::read(*this, *conn, Phantom<DrvOutputs> {});
...
void RemoteStore::collectGarbage(const GCOptions & options, GCResults & results)
{
    ...
    worker_proto::write(*this, *conn, options.pathsToDelete);
...
{
    {
        ...
        conn.processStderr();
        willBuild = worker_proto::read(*this, *conn, Phantom<StorePathSet> {});
        willSubstitute = worker_proto::read(*this, *conn, Phantom<StorePathSet> {});
        unknown = worker_proto::read(*this, *conn, Phantom<StorePathSet> {});
