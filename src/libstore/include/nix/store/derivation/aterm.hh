 ...
 * masked hashes (`masked::HashInputs`), which is the form whose hash is
 * an input address.
 *
...
concept RenderableDerivation =
    ...
    // Masked hash is only for input addressing, with masked (`Deferred`) or unmasked (`InputAddressed`) outputs
