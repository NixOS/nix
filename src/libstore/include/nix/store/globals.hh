...
public:
    ...
    template<typename S, std::enable_if_t<std::is_convertible_v<S, Path>, int> = 0>
    SandboxPath(
        S && src,
        bool optional = false,
        bool readOnly = false,
    ...
    friend void to_json(nlohmann::json &, const SandboxPath &, const Path &);
    friend void to_json(nlohmann::json &, const SandboxPath &);
};
...
struct SandboxPaths
{
    ...
    static SandboxPaths::Paths parse(const std::string_view &, const std::string & = "<unknown>");
};
