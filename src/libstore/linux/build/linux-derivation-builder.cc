    ...
    if (usingUserNamespace) {
        ...
        if (needDoubleUserns()) {
            ...
            /* In order for the unshare(CLONE_NEWUSER) that we will call in
               setUserDoubleUserns() to work, we must map our current UID and
               GID into the intermediate userns. For binfmt_misc to work, in the
               intermediate userns UID 0 and GID 0 must be mapped (see comment
               on needDoubleUserns()). This satisfies both requirements. */

            uid_map += fmt("%d %d %d\n", 0, getuid(), 1);
            gid_map += fmt("%d %d %d\n", 0, getgid(), 1);

            if (buildUser) {
                /* If using build users, we also need to map the allocated UIDs
                   and GIDs into the intermediate userns. Map them starting at
                   UID 1 and GID 1. In this case, our current UID and GID won't
                   be mapped into the final userns. */
...
/* Mounting binfmt_misc in a userns creates a new binfmt_misc instance and
   associates the user namespace with it, if there is no such existing instance.
   If an existing associated instance exists it is reused.

   From this point onward, new programs executed in this userns only look up
   binfmt_misc interpreters from the associate instance. User namespaces without
   an associated binfmt_misc instance use the same instance as its parent, if
   there is one, which may be recursively reusing the instance from some further
   ancestor.

   The files in the binfmt_misc filesystem associated with a userns are owned by
   UID 0 and GID 0 in that userns. This means that for such files to be
   writable, even to a process with CAP_DAC_OVERRIDE in that userns, UID 0 and
   GID 0 must be mapped. In the unprivileged case, in particular, since we can
   only map one UID and one GID, we can *only* map UID 0 and GID 0.

   With the uid-range feature, the build process running as UID 0 and GID 0 is
   intended, so we don't do anything special for that case. However, normally
   running builds as root is probably not a good idea. Therefore, in
   setUserDoubleUserns() we unshare(CLONE_NEWUSER) again, and map sandboxUid()
   and sandboxGid(). This way the build sees the expected UID and GID.

   However, note that if we *only* need to mount binfmt_misc, and don't need to
   write to /proc/sys/fs/binfmt_misc/register, we don't have a problem. We can
   still mount /proc/sys/fs/binfmt_misc which creates an instance associated
   with our sandbox userns, even though the files in it are completely
   inaccessible. Therefore, we also don't need this trick if binfmtMisc is
   supposed to be set up but it's empty. This is useful for opting out of the
   global binfmt_misc in Nix builds.

   See Linux commit "binfmt_misc: enable sandboxed mounts":
   https://git.kernel.org/torvalds/c/21ca59b365c0 */

bool ChrootLinuxDerivationBuilder::needDoubleUserns()
{
    return binfmtMisc && !binfmtMisc->empty() && (sandboxUid() != 0 || sandboxGid() != 0);
}

void ChrootLinuxDerivationBuilder::setUserDoubleUserns()
{
    Pipe uidMapSync;
    uidMapSync.create();
    ...
    /* At this point, we still have the original UID and GID, we're in the
       ...
       lose the privileges to write uid_map and gid_map. It's also too late to
       ask the parent process for help now. Instead, spawn a child process as a
       helper to do that. */
    ...
    Pid helper = startProcess([&]() {
        ...
        /* See comment back when the intermediate userns's uid_map and gid_map
           were written.
