        ...
        return retrySQLite<std::optional<CacheInfo>>([&]() -> std::optional<CacheInfo> {
            ...
            if (i == state->caches.end()) {
                ...
                state->caches.emplace(
                    uri, Cache{
                             (int) queryCache.getInt(0), queryCache.getStr(1), queryCache.getInt(2) != 0,
                             (int) queryCache.getInt(3)});
            }

            auto & cache(getCache(*state, uri));

            return CacheInfo{.wantMassQuery = cache.wantMassQuery, .priority = cache.priority};
        });
    }
            ...
            if (info) {
                ...
                state->insertNAR
                    .use()(cache.id)(hashPart) (std::string(info->path.name()))(
                        ...
                        narInfo && narInfo->fileHash ? narInfo->fileHash->to_string(Base32, true) : "",
                        narInfo
                            && narInfo->fileHash)(narInfo ? narInfo->fileSize : 0, narInfo != 0 && narInfo->fileSize)(
                        info->narHash.to_string(Base32, true))(info->narSize)(concatStringsSep(" ", info->shortRefs()))(
