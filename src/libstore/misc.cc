...
std::map<DrvOutput, StorePath> drvOutputReferences(Store & store, const Derivation & drv, const StorePath & outputPath)
{
    ...
    for (const auto & [inputDrv, outputNames] : drv.inputDrvs) {
        const auto outputHashes = staticOutputHashes(store, store.readDerivation(inputDrv));
        for (const auto & outputName : outputNames) {
            ...
            if (!thisRealisation)
                throw Error("output '%s' of derivation '%s' isn't built", outputName, store.printStorePath(inputDrv));
