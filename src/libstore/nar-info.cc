...
nlohmann::json UnkeyedNarInfo::toJSON(const StoreDirConfig * store, bool includeImpureInfo) const
{
    ...
    if (includeImpureInfo) {
        ...
        if (fileHash) {
            // Back compat hack, see common for "narHash" in `UnkeyedValidPathInfo::toJSON` for details.
            jsonObject["downloadHash"] = store ? static_cast<json>(fileHash->to_string(HashFormat::SRI, true)) : static_cast<json>(*fileHash);
        }
        if (fileSize)
...
void adl_serializer<UnkeyedNarInfo>::to_json(json & json, UnkeyedNarInfo c)
{
