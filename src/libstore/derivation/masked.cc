 ...
 * `fullyMaskDerivation` wants both.
 *
    ...
    const StoreDirConfig & store, ReadDerivation & readDerivation, FullInputs inputs, std::string_view drvName)
{
    ...
    for (auto & [drvPath, node] : inputs.drvs.map) {
        ...
        if (!node.childMap.empty())
            return std::nullopt;
        if (inputMasked(store, readDerivation, masked.drvs.map, drvPath, node.value, drvName))
            return std::nullopt;
    }
...
MaskedHash hashInput(const StoreDirConfig & store, ReadDerivation & readDerivation, const Full & drv)
{
    ...
    /* Extract InputAddressed outputs. If any output is not
       InputAddressed, this derivation has no masked hash. */
