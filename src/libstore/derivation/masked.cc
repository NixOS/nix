...
/* --------------------------------------------------------------------------
   Derivation masked hash
   -------------------------------------------------------------------------- */

Hashes hashes;

/* pathMaskedHash and hashInput are mutually recursive
 */
 ...
 */
static MaskedHash
pathMaskedHash(const StoreDirConfig & store, ReadDerivation & readDerivation, const StorePath & drvPath)
{
    std::optional<MaskedHash> hash;
...
{
    const auto & res = pathMaskedHash(store, readDerivation, drvPath);
    return std::visit(
        overloaded{
            [&](const MaskedHash::DeferredDrv &) { return true; },
            // Regular non-CA derivation, replace derivation
            [&](const MaskedHash::DrvHash & drvHash) {
                drvInputs[drvHash].insert(outputNames.begin(), outputNames.end());
                return false;
            },
            // CA derivation's output hashes
            [&](const MaskedHash::CaOutputHashes & outputHashes) {
...
/**
 * Input masking: replace each input derivation store path with its masked
 * hash, producing the intermediate form whose ATerm is hashed.
 *
...
maskInputDrvs(const StoreDirConfig & store, ReadDerivation & readDerivation, Derivation<FullInputs, Out> drv)
{
    ...
    for (auto & [drvPath, node] : drv.inputs.drvs.map) {
        ...
        if (!node.childMap.empty())
            return std::nullopt;
        if (inputMasked(store, readDerivation, masked.inputs.drvs.map, drvPath, node.value, masked.name))
            return std::nullopt;
    }
 ...
 * derivations.
 *
 * Returns the appropriate `MaskedHash` variant:
...
MaskedHash hashInput(const StoreDirConfig & store, ReadDerivation & readDerivation, const Full & drv)
{
    ...
    /* If any output is not InputAddressed, this derivation has no masked
       hash. */
    for (auto & [name, output] : drv.outputs)
        if (!std::get_if<Output::InputAddressed>(&output.raw))
            return MaskedHash::DeferredDrv{};

    auto inputAddressingMasked = maskInputDrvs(
        store,
        readDerivation,
    ...
    if (!inputAddressingMasked)
        return MaskedHash::DeferredDrv{};

    return hashString(HashAlgorithm::SHA256, unparse(*inputAddressingMasked, store));
}
