        ...
        std::visit(
            overloaded{
                ...
                [&](const SingleDerivedPath::Built & built) {
                    ...
                    /* Build the nested structure by finding the node for
                       `path`, recursing to the root first so that the
                       innermost output level ends up nearest the root
                       (matching `toSet`). */
                    ...
                    auto nodeFor = [&](this auto && self, ref<const SingleDerivedPath> path) -> ChildNode & {
                        return std::visit(
                            overloaded{
                                [&](const SingleDerivedPath::Opaque &) -> ChildNode & { return rootNode; },
                    ...
                    nodeFor(built.drvPath).value.insert(built.output);
                }},
            input.raw());
    }
