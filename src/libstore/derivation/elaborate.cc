...
typename derivation::ATermT<Inputs, Output>::Elaborated derivation::ATermT<Inputs, Output>::elaborate(
    const StoreDirConfig & store, std::string_view name, const ExperimentalFeatureSettings & xpSettings) const
{
    using Input = typename derivation::InputsElement<Inputs>::type;
...
template Derivation derivation::ATerm::elaborate(
    const StoreDirConfig & store, std::string_view name, const ExperimentalFeatureSettings & xpSettings) const;
template BasicDerivation derivation::BasicATerm::elaborate(
    const StoreDirConfig & store, std::string_view name, const ExperimentalFeatureSettings & xpSettings) const;
