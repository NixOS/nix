...
#ifndef _WIN32
        unix::setWriteTime(dirFd, path, st.st_atime, mtimeStore);
#else
        windows::setWriteTime(dirFd, path, st.st_atime, mtimeStore);
#endif
    }
}
    ...
    Descriptor dirFd, const CanonPath & path, CanonicalizePathMetadataOptions options, InodesSeen & inodesSeen)
{
    ...
    if (S_ISDIR(st.st_mode)) {
        AutoCloseFD childDirFd =
            unix::openFileEnsureBeneathNoSymlinks(dirFd, path, O_RDONLY | O_DIRECTORY | O_NOFOLLOW | O_CLOEXEC);
        if (!childDirFd)
