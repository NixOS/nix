...
DaemonStore::DaemonStore(const Params & params, size_t maxConnections)
    : Store(params)
...
void DaemonStore::initConnection(Connection & conn)
{
...
void DaemonStore::setOptions(Connection & conn)
{
...
bool DaemonStore::isValidPathUncached(const Path & path)
{
...
PathSet DaemonStore::queryValidPaths(const PathSet & paths)
{
    auto conn(connections->get());
    if (GET_PROTOCOL_MINOR(conn->daemonVersion) < 12) {
        PathSet res;
...
PathSet DaemonStore::queryAllValidPaths()
{
    auto conn(connections->get());
...
PathSet DaemonStore::querySubstitutablePaths(const PathSet & paths)
{
    auto conn(connections->get());
    if (GET_PROTOCOL_MINOR(conn->daemonVersion) < 12) {
        PathSet res;
...
void DaemonStore::querySubstitutablePathInfos(const PathSet & paths,
...
std::shared_ptr<ValidPathInfo> DaemonStore::queryPathInfoUncached(const Path & path)
{
    auto conn(connections->get());
...
void DaemonStore::queryReferrers(const Path & path,
    PathSet & referrers)
{
    auto conn(connections->get());
...
PathSet DaemonStore::queryValidDerivers(const Path & path)
{
    auto conn(connections->get());
...
PathSet DaemonStore::queryDerivationOutputs(const Path & path)
{
    auto conn(connections->get());
...
PathSet DaemonStore::queryDerivationOutputNames(const Path & path)
{
    auto conn(connections->get());
...
Path DaemonStore::queryPathFromHashPart(const string & hashPart)
{
    auto conn(connections->get());
...
Path DaemonStore::addToStore(const string & name, const Path & _srcPath,
...
Path DaemonStore::addTextToStore(const string & name, const string & s,
...
void DaemonStore::buildPaths(const PathSet & drvPaths, BuildMode buildMode)
{
    auto conn(connections->get());
...
BuildResult DaemonStore::buildDerivation(const Path & drvPath, const BasicDerivation & drv,
    BuildMode buildMode)
{
    auto conn(connections->get());
...
void DaemonStore::ensurePath(const Path & path)
{
    auto conn(connections->get());
...
void DaemonStore::addTempRoot(const Path & path)
{
    auto conn(connections->get());
...
void DaemonStore::addIndirectRoot(const Path & path)
{
    auto conn(connections->get());
...
void DaemonStore::collectGarbage(const GCOptions & options, GCResults & results)
{
    auto conn(connections->get());
...
void DaemonStore::optimiseStore()
{
    auto conn(connections->get());
...
bool DaemonStore::verifyStore(bool checkContents, bool repair)
{
    auto conn(connections->get());
...
void DaemonStore::addSignatures(const Path & storePath, const StringSet & sigs)
{
    auto conn(connections->get());
...
DaemonStore::Connection::~Connection()
{
    try {
        to.flush();
    } catch (...) {
...
void DaemonStore::Connection::processStderr(Sink * sink, Source * source)
{
    to.flush();
    unsigned int msg;
