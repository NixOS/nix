    ...
    const std::map<std::pair<StorePath, std::string>, StorePath> & inputDrvOutputs)
{
    auto getOutput = [&](const std::string & outputName) {
        ...
        if (!actualPathOpt)
            warn("output %s of input %s missing, aborting the resolving", outputName, store.printStorePath(inputDrv));
    ...
    for (auto & [outputName, childNode] : inputNode.childMap) {
        ...
        if (!tryResolveInput(store, inputSrcs, inputRewrites, &nextPlaceholder, actualPath, childNode, inputDrvOutputs))
            return false;
    }
    return true;
}
    ...
    Store & store, const std::map<std::pair<StorePath, std::string>, StorePath> & inputDrvOutputs) const
{
    ...
    for (auto & [inputDrv, inputNode] : inputDrvs.map)
        if (!tryResolveInput(store, resolved.inputSrcs, inputRewrites, nullptr, inputDrv, inputNode, inputDrvOutputs))
            return std::nullopt;
