...
static Machine parseBuilderLine(const std::string & line)
{
    ...
    auto parseUnsignedIntField = [&](size_t fieldIndex) {
        ...
        if (!result) {
            throw FormatError(
                "bad machine specification: failed to convert column #%lu in a row: '%s' to 'unsigned int'", fieldIndex,
                line);
        }
        return result.value();
    };
    ...
    auto ensureBase64 = [&](size_t fieldIndex) {
        ...
        try {
            ...
            throw FormatError(
                "bad machine specification: a column #%lu in a row: '%s' is not valid base64 string: %s", fieldIndex,
                line, e.what());
        }
        return str;
    };
