...
Goal::Co BuildTraceTrampolineGoal::init()
{
    ...
    if (drvOpt) {
        ...
        drvIsResolved = {std::ranges::none_of(drv.inputs, [](const auto & input) {
    ...
    auto g = worker.makeDerivationResolutionGoal(id2.drvPath, make_ref<const Derivation>(*drvOpt), bmNormal);
