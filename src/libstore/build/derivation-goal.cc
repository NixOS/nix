...
Goal::Co DerivationGoal::haveDerivation()
{
    ...
    if (!waitees.empty())
        co_await Suspend{}; /* to prevent hang (no wake-up event) */
...
Goal::Co DerivationGoal::gaveUpOnSubstitution()
{
    ...
    if (useDerivation) {
        ...
        addWaiteeDerivedPath = [&](ref<SingleDerivedPath> inputDrv,
    ...
    for (auto & outputName : outputs) {
        ...
        if (buildResult.success()) {
            ...
            if (i != buildResult.builtOutputs.end())
                inputDrvOutputs.insert_or_assign({dg->drvPath, outputName}, i->second.outPath);
        }
    }
}
