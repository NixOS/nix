...
DrvOutputSubstitutionGoal::DrvOutputSubstitutionGoal(const DrvOutputId& id, Worker & worker, RepairFlag repair, std::optional<ContentAddress> ca)
    : Goal(worker)
    , id(id)
{
...
void DrvOutputSubstitutionGoal::tryNext()
{
    ...
    // FIXME: Make async
    outputInfo = sub->queryDrvOutputInfo(id);
    if (!outputInfo) {
        tryNext();
        return;
    }
...
string DrvOutputSubstitutionGoal::key()
{
    /* "a$" ensures substitution goals happen before derivation
       goals. */
    return "a$" + std::string(id.drvPath.name()) + "$" + id.to_string();
}
