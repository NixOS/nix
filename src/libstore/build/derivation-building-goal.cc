    ...
    const SingleDrvOutputs & builtOutputs)
{
    ...
    hookEnvironment.insert_or_assign(OS_STR("NIX_CONFIG"), string_to_os_string(globalConfig.toKeyValue()));
    hookEnvironment.insert_or_assign(OS_STR("DRV_ATERM_FD"), string_to_os_string(std::to_string(drvAtermFdNum)));
    hookEnvironment.insert_or_assign(
        OS_STR("BUILD_INFO_JSON_FD"), string_to_os_string(std::to_string(buildInfoJsonFdNum)));
