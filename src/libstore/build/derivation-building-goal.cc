...
Goal::Co DerivationBuildingGoal::tryToBuild()
{
    ...
    auto & [resolvedDrv, resolvedDrvOptions] = *resolvedDrvOpt;
    ...
    while (true) {
        ...
        bool buildLocally = (buildMode != bmNormal || resolvedDrvOptions.willBuildLocally(worker.store, resolvedDrv))
                            && settings.maxBuildJobs.get() != 0;

        if (buildLocally) {
            useHook = false;
        } else {
            switch (tryBuildHook(initialOutputs, resolvedDrv)) {
            case rpAccept:
            ...
            case rpDecline:
                ...
                if (!externalBuilder && !resolvedDrvOptions.canBuildLocally(worker.store, resolvedDrv)) {
                    auto msg =
                        fmt("Cannot build '%s'.\n"
                            ...
                            concatStringsSep(", ", resolvedDrvOptions.getRequiredSystemFeatures(resolvedDrv)),
    ...
    while (true) {
        ...
        if (!builder) {
            ...
            auto drvOptionsResolved = [&] {
                ...
                /* The derivation must have all of its inputs gotten this point,
                   so the resolution will sure succeed.
...
HookReply DerivationBuildingGoal::tryBuildHook(
    const std::map<std::string, InitialOutput> & initialOutputs, const BasicDerivation & resolvedDrv)
{
    ...
    try {
        ...
        worker.hook->sink << "try" << (worker.getNrLocalBuilds() < settings.maxBuildJobs ? 1 : 0) << drv->platform
                          << worker.store.printStorePath(drvPath)
                          << resolvedDrv.options.getRequiredSystemFeatures(resolvedDrv);
