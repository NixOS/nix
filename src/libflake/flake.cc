...
static Flake
getFlake(EvalState & state, const FlakeRef & originalRef, bool useRegistries, const InputAttrPath & lockRootAttrPath)
{
                        ...
                        if (input.isFlake) {
                            ...
                            auto [path, lockedRef] = [&]() -> std::tuple<SourcePath, FlakeRef> {
                                ...
                                if (auto resolvedPath = resolveRelativePath()) {
                                    ...
                                    auto cachedInput =
                                        state.inputCache->getAccessor(state.store, input.ref->input, useRegistries);
