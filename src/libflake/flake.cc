...
static std::tuple<ref<SourceAccessor>, FlakeRef, FlakeRef> fetchOrSubstituteTree(
    EvalState & state, const FlakeRef & originalRef, fetchers::UseRegistries useRegistries, FlakeCache & flakeCache)
{
    ...
    const InputAttrPath & lockRootAttrPath)
{
    ...
    auto [accessor, resolvedRef, lockedRef] = fetchOrSubstituteTree(state, originalRef, useRegistries, flakeCache);
    ...
    if (lockedRef != newLockedRef) {
        ...
        auto [accessor2, resolvedRef2, lockedRef2] =
            fetchOrSubstituteTree(state, newLockedRef, fetchers::UseRegistries::No, flakeCache);
        accessor = accessor2;
...
lockFlake(const Settings & settings, EvalState & state, const FlakeRef & topRef, const LockFlags & lockFlags)
{
    ...
    auto flake = getFlake(state, topRef, useRegistriesTop, flakeCache, {});
                        ...
                        if (input.isFlake) {
                            ...
                            auto [path, lockedRef] = [&]() -> std::tuple<SourcePath, FlakeRef> {
                                ...
                                if (auto resolvedPath = resolveRelativePath()) {
                                    ...
                                    auto [accessor, resolvedRef, lockedRef] =
                                        fetchOrSubstituteTree(state, *input.ref, useRegistriesInputs, flakeCache);
