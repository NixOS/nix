...
static void prim_genericClosure(EvalState & state, const PosIdx pos, Value * * args, Value & v)
{
    ...
    auto workSet = state.allocList();
    for (auto elem : startSet->value->list())
        *workSet = workSet->push_back(elem);
    ...
    for (auto & vElem : args[1]->list()) {
        ...
        for (auto & attr : *vElem->attrs()) {
            if (!attrsSeen.contains(attr.name)) {
                attrsSeen[attr.name] = ValueVector();
            }
            attrsSeen[attr.name].push_back(attr.value);
        }
    }

    auto attrs = state.buildBindings(attrsSeen.size());

    for (auto & [sym, valueVector] : attrsSeen) {
        ...
        auto list = state.allocList();
        for (auto v : valueVector) *list = list->push_back(v);
        arg->mkList(list);
...
static void prim_groupBy(EvalState & state, const PosIdx pos, Value * * args, Value & v)
{
    ...
    for (auto vElem : args[1]->list()) {
        ...
        if (!attrsSeen.contains(sym)) {
            attrsSeen[sym] = ValueVector();
        }
        attrsSeen[sym].push_back(vElem);
    }

    auto attrs2 = state.buildBindings(attrsSeen.size());

    for (auto & [name, valueVector] : attrsSeen) {
        auto list = state.allocList();
        for (auto v : valueVector) *list = list->push_back(v);
...
void prim_match(EvalState & state, const PosIdx pos, Value * * args, Value & v)
{
    ...
    try {
        ...
        for (auto match : std::span(matches).subspan(1)) {
            if (!match.matched) {
                *listOfMatches = listOfMatches->push_back(&state.vNull);
            } else {
                *listOfMatches = listOfMatches->push_back(mkString(state, match));
            }
        }
