...
public:
    ...
    public:
        ...
        Origin(std::string file, FileOrigin origin)
            : file(std::move(file))
            , origin(origin)
        {}
    };
...
struct ExprSelect : Expr
{
    ...
    Expr *e, *def;
    AttrPath attrPath;
    ExprSelect(const PosIdx & pos, Expr * e, const AttrPath & attrPath, Expr * def)
        : pos(pos)
        , e(e)
        , def(def)
        , attrPath(attrPath){};
...
struct ExprOpHasAttr : Expr
{
    Expr * e;
    AttrPath attrPath;
    ExprOpHasAttr(Expr * e, const AttrPath & attrPath)
        : e(e)
        , attrPath(attrPath){};
    PosIdx getPos() const override
    {
...
struct StaticEnv
{
    ...
    StaticEnv(bool isWith, const StaticEnv * up, size_t expectedSize = 0)
        : isWith(isWith)
        , up(up)
    {
