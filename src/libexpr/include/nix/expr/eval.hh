        ...
        return std::visit(
            overloaded{
                [&](RangeIdxs idx) {
                    ...
                    if (!idx.start)
                        idx.start = expr.pos.start;
                    return table[idx.start];
                },
                [&](Pos pos) { return pos; },
            },
            pos);
...
public:
    ...
    inline bool evalBool(Env & env, Expr * e, RangeIdxs pos, std::string_view errorCtx);
    inline void evalAttrs(Env & env, Expr * e, Value & v, RangeIdxs pos, std::string_view errorCtx);
     ...
     */
    inline void forceValue(Value & v, RangeIdxs pos);
...
private:
     ...
     */
    void handleEvalExceptionForThunk(Env * env, Expr * expr, Value & v, RangeIdxs pos);
    ...
    void handleEvalFailed(Value & v, RangeIdxs pos);
...
public:
     ...
     */
    NixInt forceInt(Value & v, RangeIdxs pos, std::string_view errorCtx);
    NixFloat forceFloat(Value & v, RangeIdxs pos, std::string_view errorCtx);
    bool forceBool(Value & v, RangeIdxs pos, std::string_view errorCtx);

    void forceAttrs(Value & v, RangeIdxs pos, std::string_view errorCtx);

    template<typename Callable>
    void forceAttrs(Value & v, Callable getPos, std::string_view errorCtx);

    inline void forceList(Value & v, RangeIdxs pos, std::string_view errorCtx);
    /**
     * @param v either lambda or primop
     */
    void forceFunction(Value & v, RangeIdxs pos, std::string_view errorCtx);
    std::string_view forceString(Value & v, RangeIdxs pos, std::string_view errorCtx);
    std::string_view forceString(
        ...
        const ExperimentalFeatureSettings & xpSettings = experimentalFeatureSettings);
    std::string_view forceStringNoCtx(Value & v, RangeIdxs pos, std::string_view errorCtx);
    ...
    template<typename... Args>
    [[gnu::noinline]]
    void addErrorTrace(Error & e, RangeIdxs pos, const Args &... formatArgs) const;
...
public:
    ...
    std::optional<std::string> tryAttrsToString(
        RangeIdxs pos, Value & v, NixStringContext & context, bool coerceMore = false, bool copyToStore = true);
     ...
     */
    SourcePath coerceToPath(RangeIdxs pos, Value & v, NixStringContext & context, std::string_view errorCtx);

    /**
     * Like coerceToPath, but the result must be a store path.
     */
    StorePath coerceToStorePath(RangeIdxs pos, Value & v, NixStringContext & context, std::string_view errorCtx);
     ...
     */
    SingleDerivedPath coerceToSingleDerivedPath(RangeIdxs pos, Value & v, std::string_view errorCtx);
...
public:
     ...
     */
    inline CallDepth addCallDepth(RangeIdxs pos);

    /**
     * Do a deep equality test between two values.  That is, list
     * elements and attributes are compared recursively.
     */
    bool eqValues(Value & v1, Value & v2, RangeIdxs pos, std::string_view errorCtx);
     ...
     */
    void assertEqValues(Value & v1, Value & v2, RangeIdxs pos, std::string_view errorCtx);

    bool isFunctor(const Value & fun) const;

    void callFunction(Value & fun, std::span<Value *> args, Value & vRes, RangeIdxs pos);

    void callFunction(Value & fun, Value & arg, Value & vRes, RangeIdxs pos)
    {
        Value * args[] = {&arg};
     ...
     */
    void concatLists(Value & v, std::span<Value * const> lists, RangeIdxs pos, std::string_view errorCtx);
