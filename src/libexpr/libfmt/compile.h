...
/**
  ...
  **Example**::
    ...
    std::string s = libfmt::format(FMT_COMPILE("{}"), 42);
  \endrst
 */
#ifdef __cpp_if_constexpr
#  define FMT_COMPILE(s) FMT_STRING_IMPL(s, libfmt::detail::compiled_string)
#else
...
constexpr parse_specs_result<T, Char> parse_specs(basic_string_view<Char> str,
  ...
  return {f, pos + libfmt::detail::to_unsigned(end - str.data()) + 1,
  ...
  if constexpr (std::is_same<typename S::char_type, char>::value) {
    ...
    if constexpr (str.size() == 2 && str[0] == '{' && str[1] == '}') {
      ...
      if constexpr (detail::is_named_arg<
        ...
        return libfmt::to_string(first.value);
      } else {
        return libfmt::to_string(first);
      }
    }
  }
