...
#    define FMT_ASSERT(condition, message)                                    \
      ((condition) /* void() fails with -Winvalid-constexpr on clang 4.0.1 */ \
           ? (void)0                                                          \
           : ::libfmt::detail::assert_fail(__FILE__, __LINE__, (message)))
#  endif
#endif
...
/**
  ...
  subset of the API. ``libfmt::basic_string_view`` is used for format strings even
...
/**
  ...
  **Example**::
    ...
    std::string message = libfmt::format(my_string("The answer is {}"), 42);
  \endrst
 */
...
namespace detail {
void to_string_view(...);
using libfmt::v7::to_string_view;

// Specifies whether S is a string type convertible to libfmt::basic_string_view.
...
/**
  ...
  class and shouldn't be used directly, only via `~libfmt::basic_memory_buffer`.
  \endrst
 */
...
/**
  ...
  `~libfmt::basic_format_args` for passing into type-erased formatting functions
  such as `~libfmt::vformat`.
  \endrst
 */
...
/**
  \rst
  Constructs a `~libfmt::format_arg_store` object that contains references to
  arguments and can be implicitly converted to `~libfmt::format_args`. `Context`
  can be omitted in which case it defaults to `~libfmt::context`.
  See `~libfmt::arg` for lifetime considerations.
  \endrst
 */
...
/**
  \rst
  Constructs a `~libfmt::format_arg_store` object that contains references
  to arguments and can be implicitly converted to `~libfmt::format_args`.
...
/**
  ...
  **Example**::
    ...
    libfmt::print("Elapsed time: {s:.2f} seconds", libfmt::arg("s", 1.23));
  \endrst
 */
...
template <typename Context> class basic_format_args {
 ...
 public:
  ...
  /**
   \rst
   Constructs a `basic_format_args` object from `~libfmt::format_arg_store`.
   \endrst
   */
  template <typename... Args>
  ...
  /**
   ...
   `~libfmt::dynamic_format_arg_store`.
   \endrst
   */
  constexpr FMT_INLINE basic_format_args(
 ...
 **Example**::
   ...
   std::vector<char> out;
   libfmt::format_to(std::back_inserter(out), "{}", 42);
 \endrst
 */
...
inline auto format_to(OutputIt out, const S& format_str, Args&&... args) ->
    typename std::enable_if<enable, OutputIt>::type {
  const auto& vargs = libfmt::make_args_checked<Args...>(format_str, args...);
...
/**
  ...
  **Example**::
    ...
    #include <fmt/core.h>
    std::string message = libfmt::format("The answer is {}", 42);
  \endrst
*/
...
/**
  ...
  **Example**::
    ...
    libfmt::print(stderr, "Don't {}!", "panic");
  \endrst
 */
template <typename S, typename... Args, typename Char = char_t<S>>
...
/**
  ...
  **Example**::
    ...
    libfmt::print("Elapsed time: {0:.2f} seconds", 1.23);
  \endrst
 */
template <typename S, typename... Args, typename Char = char_t<S>>
