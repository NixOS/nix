 ...
 *
 * Also gives you the name. Attributes are returned in implementation-defined order
 * and are NOT suitable for reproducible operations. In Nix's domain, reproducibility
 * is paramount. The caller is responsible for sorting the attributes or storing them
 * in an ordered map to ensure deterministic behavior in your application.
 * (Nix normally sorts attributes in byte-wise lexicographic order, which, for valid
 * UTF-8, is equivalent to lexicographic order by Unicode scalar value.)
 *
 * Use nix_gc_decref when you're done with the pointer
 * @param[out] context Optional, stores error information
 * @param[in] value Nix value to inspect
 ...
 * The attribute set value itself must already be evaluated.
 *
 * Attributes are returned in implementation-defined order and are NOT suitable for
