...
Derivation * nix_derivation_from_json(nix_c_context * context, StoreDir store_dir, const char * json)
{
    if (context)
        context->last_err_code = NIX_OK;
    try {
    ...
    const StorePath * inputPaths[])
{
    ...
    try {
        ...
        void * p = ::operator new(sizeof(DerivationBuilder));
        auto & ref = *static_cast<DerivationBuilder *>(p);
        return new (p) DerivationBuilder{
