...
struct JjInputScheme : InputScheme
{
        ...
        const Settings & settings, Input & input, const std::filesystem::path & repoPath, const Metadata & meta) const
    {
        ...
        /* Skip lastModified/revCount if they're already supplied by the caller
           (e.g. from a lock file). Crucially, overwriting a locked `revCount`
           (a plain integer) with our `LazyAttr` here would make `checkLocks`
           see two different `Attr` variants and wrongly report a mismatch. */
    ...
    getAccessorFromRemote(const Settings & settings, Store & store, Input input) const
    {
        ...
        auto makeResult = [&](const Attrs & infoAttrs, const StorePath & storePath, const Hash & rev)
            -> std::pair<ref<SourceAccessor>, Input> {
            input.attrs.insert_or_assign("rev", rev.gitRev());
            /* Skip lastModified/revCount if already supplied by the caller (e.g.
               from a lock file); see the note in `setAttrs`. */
            if (!input.attrs.contains("lastModified"))
