    ...
    Setting<StringMap> accessTokens{this, {}, "access-tokens",
        R"(
          ...
          * Gitlab: the token value is one of an OAuth2 token, a
            Personal Access Token, or a CI_JOB_TOKEN (these are different
            ...
            `type` is either `OAuth2`, `PAT` or `CI_JOB_TOKEN` to indicate
            which type of token is being specified.
          ...
          `CI_JOB_TOKENS` are ephemeral tokens generated by GitLab CI and
          are unique to each invocation of a GitLab CI build. Because of this,
          the easiest way to use them is to set the `NIX_CONFIG` environment
          variable:
          ...
          script:
            ...
            ...
          ```
          or append the setting to the runner's `nix.conf` before invoking Nix.
