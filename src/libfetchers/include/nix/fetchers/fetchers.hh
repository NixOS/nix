...
public:
     ...
     */
    std::pair<StorePath, Input> fetchToStore(const FetchContext & context, Store & store) const;
     ...
     * unlocked input.
     */
    std::pair<ref<SourceAccessor>, Input> getAccessor(const FetchContext & context, Store & store) const;

private:

    std::pair<ref<SourceAccessor>, Input> getAccessorUnchecked(const FetchContext & context, Store & store) const;

public:

    Input applyOverrides(std::optional<std::string> ref, std::optional<Hash> rev) const;

    void clone(const FetchContext & context, Store & store, const std::filesystem::path & destDir) const;
...
struct InputScheme
{
    ...
    getAccessor(const FetchContext & context, Store & store, const Input & input) const = 0;
    ...
    getAccessToken(const FetchContext & context, const std::string & host, const std::string & url) const
    {
        return {};
    }
};
