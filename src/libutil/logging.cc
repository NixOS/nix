...
{
    return std::unique_ptr<Logger>(new SimpleLogger(printBuildLogs));
}
...
std::unique_ptr<Logger> makeJSONLogger(std::shared_ptr<Logger> prevLogger)
{
    return std::unique_ptr<Logger>(new JSONLogger(prevLogger));
}
