...
static ParsedURL fromBoostUrlView(boost::urls::url_view urlView, bool lenient)
{
    if (!urlView.has_scheme())
        throw BadURL("'%s' doesn't have a scheme", static_cast<boost::core::string_view>(urlView));
    ...
    if (authority && authority->host.size() && transportIsFile)
        throw BadURL(
            "file:// URL '%s' has unexpected authority '%s'",
            static_cast<boost::core::string_view>(urlView),
            *authority);
