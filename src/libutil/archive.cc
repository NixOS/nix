...
void SourceAccessor::dumpPath(const CanonPath & path, Sink & sink, PathFilter & filter)
{
    ...
    std::function<void(const CanonPath & path, size_t depth)> dump;

    dump = [&](const CanonPath & path, size_t depth) {
        checkInterrupt();

        if (depth >= narMaxDepth)
            throw Error("path '%s' exceeds maximum NAR directory depth of %d", showPath(path), narMaxDepth);
...
static void parse(FileSystemObjectSink & sink, Source & source, const CanonPath & path, size_t depth)
{
    ...
    if (type == "regular") {
        ...
        while (1) {
            ...
            parse(sink, source, path / name, depth + 1);
