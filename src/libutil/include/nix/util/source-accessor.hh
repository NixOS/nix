...
#define MakeSymlinkError(errorName, defaultMsg)                                        \
    ...
    {                                                                                  \
        std::variant<std::filesystem::path, CanonPath> path;                           \
        errorName(std::filesystem::path path)                                          \
            : CloneableError(defaultMsg, PathFmt(path))                                \
            , path(std::move(path))                                                    \
        {                                                                              \
        }                                                                              \
        errorName(CanonPath path)                                                      \
            : CloneableError(defaultMsg, path.rel())                                   \
            , path(std::move(path))                                                    \
        {                                                                              \
        }                                                                              \
        ...
        template<typename... Args>                                                     \
        errorName(CanonPath path, const std::string & fs, Args &&... args)             \
            : CloneableError(fs, std::forward<Args>(args)...)                          \
            , path(std::move(path))                                                    \
        {                                                                              \
        }                                                                              \
