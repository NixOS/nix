...
public:
    ...
    /**
     * Convert back to the portable `CanonPath`, the inverse of the
     * constructor above.
     *
     * Structurally this round-trips: `OsCanonPath` invariants mean every
     * component is a plain filename, which is exactly what `CanonPath`
     * accepts, so no component is dropped or renormalized.
     *
     * @todo The *encoding* does not round-trip on Windows, where this
     * narrows each `wchar_t` component. Names that are not valid Unicode
     * (e.g. unpaired surrogates) will not survive until we represent
     * `CanonPath` as WTF-8.
     */
    CanonPath toPortable() const;
    ...
    /**
     * Remove the last component. Panics if this path is empty.
     */
    void pop();

    /**
     * Everything but the last component, or `std::nullopt` if this path
     * is empty.
     */
    std::optional<OsCanonPath> parent() const;

    /**
     * Whether `prefix` is this path or an ancestor of it.
     */
    bool isWithin(const OsCanonPath & prefix) const;

    /**
     * Reinterpret this path as relative to `prefix` rather than to
     * whatever it was relative to before. Panics unless
     * `isWithin(prefix)`.
     */
