    ...
    std::string_view stem, const Derivation & drv, const StructuredAttrs * parsed)
{
    ...
    drvWithOpts = derivation::elaborate(derivation::lower(drvWithOpts), *store, drvWithOpts.name);
    ...
    auto resolved = tryResolve(drvWithOpts, *store, makeCallback(buildTrace));
    ASSERT_TRUE(resolved);
