 ...
 * `hashInput`.
 *
 * The point of "masked hash" is that derivations which differ only in
 ...
 *   the intermediates are indistinguishable once masked, these two must have
 *   the same masked hash.
 */
class MaskedHashTest : public virtual CharacterizationTest
{
...
protected:
     ...
     * masked hash) does not depend on `builder`.
     */
    ...
    MaskedHash inputHash(Written & written, const Full & drv)
    {
        auto readDerivation = readDrv(written);
        return hashInput(store, readDerivation, drv);
    }
...
struct MaskedHashJsonTest : MaskedHashTest,
                            JsonCharacterizationTest<Full>,
                            ::testing::WithParamInterface<std::string_view>
{};

TEST_P(MaskedHashJsonTest, from_json)
{
    Written written;
    readJsonTest(GetParam(), named(written, GetParam()));
}

TEST_P(MaskedHashJsonTest, to_json)
{
    Written written;
    writeJsonTest(GetParam(), named(written, GetParam()));
}

INSTANTIATE_TEST_SUITE_P(MaskedHashJSON, MaskedHashJsonTest, ::testing::ValuesIn(MaskedHashTest::stems));
...
INSTANTIATE_TEST_SUITE_P(MaskedHashATerm, MaskedHashATermTest, ::testing::ValuesIn(MaskedHashTest::stems));
 ...
 *
 * The fixed-output `source-*` derivations have no single masked hash,
 ...
 * derivations that are nonetheless identical with their fixed-output
 * inputs masked, which is what makes `parent-split` interesting: its two input
 ...
 * clobbered.
 */
TEST_F(MaskedHashTest, collidingInputDrvsMergeOutputNames)
{
    Written written;
 ...
 */
TEST_F(MaskedHashTest, differingOutputNamesDiffer)
{
    Written written;
...
/**
 * A fixed-output derivation's masked hash is a map of output hashes,
 * and does not depend on how the output is produced. This is the
 * premise the collision above rests on.
 */
TEST_F(MaskedHashTest, fixedOutputIsProvenanceFree)
{
    Written written;
    auto first = inputHash(written, named(written, "source-first"));
    auto second = inputHash(written, named(written, "source-second"));

    auto * outputHashes = std::get_if<MaskedHash::CaOutputHashes>(&first.raw);
 ...
 * different masked hashes, and so their dependents do too.
 */
TEST_F(MaskedHashTest, differingInputDrvsDiffer)
{
    Written written;
