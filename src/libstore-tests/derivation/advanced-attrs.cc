...
static ParsedOptions parsedOptions(const Derivation & d)
{
    ...
    for (auto & [outputName, output] : d.outputs)
        /* Only non-default options, so expectations need not enumerate
           every output. */
        if (output.options != derivation::OutputOptions<SingleDerivedPath>{})
...
public:
        ...
        const std::string & fileName, const ParsedOptions & expected, const StringSet & expectedSystemFeatures)
    {
        this->readTest(fileName, [&](auto encoded) {
            ...
            /* In the non-structured case, checks can only be specified
               for all outputs at once; the per-output checks are always
               absent. */
            if (!got.structuredAttrs)
                for (auto & [outputName, output] : got.outputs)
                    EXPECT_EQ(output.options.checks, std::nullopt);
        });
    }
};
...
#define TEST_ATERM_JSON(STEM, NAME)                                                                        \
    ...
    {                                                                                                      \
        ...
        this->readTest("json-5/" NAME ".json", [&](const auto & encoded_) {                                \
    ...
    {                                                                                                      \
        ...
        this->readTest(NAME ".drv", [&](auto encoded) {                                                    \
            ...
            auto j = json::parse(readFile(this->goldenMaster("json-5/" NAME ".json")));                    \
...
/* Like the above, but for the legacy V4 JSON format, in which the
   derivation options are only present via their legacy
   environment-variable (and structured-attributes) encoding. Decoding
   it accordingly needs a store dir, so `derivation::fromJSON` is used
   instead of the plain JSON decoder. */
#define TEST_ATERM_JSON_V4(STEM, NAME)                                                                     \
    TYPED_TEST(DerivationAdvancedAttrsBothTest, Derivation_##STEM##_from_json_v4)                          \
    {                                                                                                      \
        using namespace nlohmann;                                                                          \
        this->readTest("json-4/" NAME ".json", [&](const auto & encoded_) {                                \
            auto encoded = json::parse(encoded_);                                                          \
            /* Use DRV file instead of C++ literal as source of truth. */                                  \
            auto aterm = readFile(this->goldenMaster(NAME ".drv"));                                        \
            auto expected = derivation::parse(*this->store, std::move(aterm), NAME, this->mockXpSettings); \
            ...
            EXPECT_EQ(got, expected);                                                                      \
                                                                                                           ...
                                                                                                           \
    TYPED_TEST(DerivationAdvancedAttrsBothTest, Derivation_##STEM##_to_json_v4)                            \
    {                                                                                                      \
        using namespace nlohmann;                                                                          \
        this->writeTest(                                                                                   \
            "json-4/" NAME ".json",                                                                        \
            [&]() -> json {                                                                                \
                /* Use DRV file instead of C++ literal as source of truth. */                              \
                auto aterm = readFile(this->goldenMaster(NAME ".drv"));                                    \
                return derivation::toJSON(                                                                 \
                    derivation::parse(*this->store, std::move(aterm), NAME, this->mockXpSettings),         \
                    derivation::JsonFormat::V4);                                                           \
            },                                                                                             \
            [](const auto & file) { return json::parse(readFile(file)); },                                 \
            [](const auto & file, const auto & got) { return writeFile(file, got.dump(2) + "\n"); });      \
    }

TEST_ATERM_JSON_V4(advancedAttributes, "advanced-attributes-defaults");
TEST_ATERM_JSON_V4(advancedAttributes_defaults, "advanced-attributes");
TEST_ATERM_JSON_V4(advancedAttributes_structuredAttrs, "advanced-attributes-structured-attrs-defaults");
TEST_ATERM_JSON_V4(advancedAttributes_structuredAttrs_defaults, "advanced-attributes-structured-attrs");

#undef TEST_ATERM_JSON_V4
...
static const ParsedOptions advancedAttributes_defaults = {
    .top =
        {
            /* Trivial legacy checks are canonicalized to no checks at
               all, so the ATerm round-trip is stable. */
...
#define TEST_JSON_OUTPUT_OPTIONS(FIXUTURE, INPUT, VAR)                                    \
    ...
    {                                                                                     \
        nix::readJsonTest<derivation::OutputOptions<INPUT>>(                              \
            *this, "derivation-output-checks/" #VAR, outputOptions_##VAR);                \
    }                                                                                     \
    TEST_F(FIXUTURE, OutputOptions_##INPUT##_##VAR##_to_json)                             \
    {                                                                                     \
        nix::writeJsonTest(*this, "derivation-output-checks/" #VAR, outputOptions_##VAR); \
    }
...
#define SYNC_CONFLICT(NAME, VALUE)                         \
    NAME = VALUE;                                          \
    EXPECT_THROW(derivation::unparse(got, *store), Error); \
    got.options = options;
...
TEST_F(DerivationAdvancedAttrsTest, Derivation_advancedAttributes_option_syncConflict)
{
    readTest("advanced-attributes-defaults.drv", [&](auto encoded) {
        auto got = derivation::parse(*store, std::move(encoded), "foo");
