    ...
    bool executable)
{
    ...
    if (!storePath) {
        ...
        if (unpack) {
            Activity act(*logger, Verbosity::Chatty, ActivityType::Unknown, fmt("unpacking '%s'", url.to_string()));
        ...
        Activity act(
            *logger, Verbosity::Chatty, ActivityType::Unknown, fmt("adding '%s' to the store", url.to_string()));
