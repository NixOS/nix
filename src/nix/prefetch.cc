...
std::string resolveMirrorUrl(EvalState & state, const std::string & url)
{
    ...
    std::string mirror(state.forceString(*mirrorList->value->list()[0], noPos, "while evaluating the first available mirror"));
...
{
    {
        ...
        if (!fromExpr) {
            ...
            url = state->forceString(*attr->value->list()[0], noPos, "while evaluating the first url from the urls list");
