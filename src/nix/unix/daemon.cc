    ...
    std::visit(
        overloaded{
            ...
            [&](UnixSocket socketPathOverride) {
                auto socketPath =
                    std::move(socketPathOverride)
                        .or_else([&]() -> std::optional<std::filesystem::path> { return getDaemonSocketPath(*storeConfig); })
                        .value();

                if (auto * udsConfig = dynamic_cast<const UDSRemoteStoreConfig *>(storeConfig.get());
                    udsConfig && udsConfig->path == socketPath)
                    warn("daemon socket path %s is the same as the store's socket path; this will fail", PathFmt(socketPath));
...
struct CmdDaemon : StoreConfigCommand
{
    ...
    CmdDaemon()
    {
        ...
        addFlag({
            ...
            .labels = {"path"},
            .handler = {[&](std::filesystem::path s) { socketPath = s; }},
        });
