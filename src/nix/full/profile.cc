...
struct CmdProfileInstall : AbstractInstallablesCommand, virtual SourceExprCommand, MixDefaultProfile
{
    ...
    void run(ref<Store> store, Installables && installables) override
    {
        ...
        auto builtPaths = builtPathsPerInstallable(
            Installable::build2(
                getDrvStore(), store, Realise::Outputs, installables, bmNormal));
        ...
        for (auto & installable : installables) {
            ...
            element.updateStorePaths(getDrvStore(), store, res);
