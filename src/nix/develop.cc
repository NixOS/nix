...
void envSetup(
    ...
    std::optional<StorePath> devShell,
    ...
    StringMap rewrites)
{
    ...
    setEnv("NIX_BUILD_TOP", tmpDir.c_str());
    ...
    std::string rewritesStr;
    for (auto & [from, to] : rewrites) {
        rewritesStr += nix::fmt("%s %s\n", from, to);
    }
    setEnv("NIX_DEVSHELL_REWRITES", rewritesStr.c_str());
}

std::string legacyDevshellScript(
    std::optional<std::string> phase,
    std::vector<std::string> command,
    DevelopSettings settings,
    std::filesystem::path rcFilePath)
{
    std::string script;
...
#include "develop.md"
    ...
    void run(ref<Store> store, ref<Installable> installable) override
    {
        ...
        if (devShell) {
            ...
            envSetup(store, phase, installable, command, devShell, tmpDir.path(), rewrites);
        } else {
            ...
            script += legacyDevshellScript(phase, command, developSettings, rcFilePath);
...
#include "print-dev-env.md"
        ...
        void run(ref<Store> store, ref<Installable> installable) override
        {
            ...
            if (devShell) {
                auto file = CanonPath{store->printStorePath(*devShell)} / (json ? "env.json" : "env");
                if (!store->getFSAccessor()->pathExists(file))
                    throw Error("%s's devShell does not provide %s", installable->what(), *file.baseName());

                logger->stop();
                logger->writeToStdout(store->getFSAccessor()->readFile(file));
            } else {
                /* Legacy devshell path*/
