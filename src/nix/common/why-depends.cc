...
struct CmdWhyDepends : virtual DrvCommand, GetRawInstallables, MixDefaultParseInstallableArgs, MixOperateOnOptions
{
    ...
    CmdWhyDepends()
        ...
        , MixOperateOnOptions(static_cast<DrvCommand &>(*this))
    {
        expectArgs({
            .label = "package",
            .handler = {&_package},
    ...
    {
        return { _package, _dependency };
    }

    void run(ref<Store> store) override
    {
        auto package = parseInstallable(store, _package);
        auto packagePath = Installable::toStorePath(getDrvStore(), store, Realise::Outputs, operateOn, package);
        ...
        auto optDependencyPath = [&]() -> std::optional<StorePath> {
            try {
                return {Installable::toStorePath(getDrvStore(), store, Realise::Derivation, operateOn, dependency)};
